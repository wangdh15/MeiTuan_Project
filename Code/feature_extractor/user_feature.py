...
class user_feature_extractor:
    ...
    def feature_1(self, recompu=False):
        '''
        对输入的origin_data进行处理，得到feature_1，然后将数据返回，主键是user_id
        ...
        if not recompu and os.path.exists(self.config.feature_file_path):
            feature_1 = pd.read_csv(self.config.feature_file_path)
            return feature_1
        else:
            # 用origin_data重新计算特征
            feature_1 = None
    ...
    def feature_2(self, recompu=False):
        ...
        if not recompu and os.path.exists(self.config.feature_file_path):
            feature_2 = pd.read_csv(self.config.feature_file_path)
            return feature_2
        else:
            # 用origin_data重新计算特征
            feature_2 = None
